package design
{
	...
	public class MenuManager extends MovieClip
	{
		...
		{
			trace("menumanager.updatecurrentmenu");
			for (var i:int=0; i < currentMenuSelected.length; i++)
			{
				trace(currentMenuSelected[i]);
				for (var p:int=0; p < currentMenuSelected[i].length; p++)
				{
					...
					{
						/*trace("1.",ct);
						trace("2.",getDefinitionByName(getQualifiedSuperclassName(ct)));
						trace("3.",getQualifiedSuperclassName(ct));*/

						//menuIcon = new MenuIcon(ct);
		...
		public function updateMenu(e:Event):void
		{
			...
			switch (true)
			{
				case (e.currentTarget is Tower) :
					...
					trace("asdf",e.currentTarget.tMenu);
					break;
			}
			updateCurrentMenu();
		}
